...
async function realizarConsultas() {
  try {
    ...
    resultEstudiantesMayores.rows.forEach((row) => {
      console.log(`ID: ${row.id}, Nombre: ${row.nombres}, Apellido: ${row.apellidos}, Edad: ${row.edad}`);
    });
    ...
    resultEstudiantesOrdenados.rows.forEach((row) => {
      console.log(`ID: ${row.id}, Nombre: ${row.nombres}, Apellido: ${row.apellidos}`);
    });

    // 3. Seleccionar todos los cursos disponibles
    const resultCursos = await pool.query('SELECT * FROM cursos;');
    console.log('\nCursos disponibles:');
    resultCursos.rows.forEach((row) => {
      console.log(`ID: ${row.id}, Título: ${row.titulo}, Descripción: ${row.descripcion}`);
    });
  } catch (err) {
