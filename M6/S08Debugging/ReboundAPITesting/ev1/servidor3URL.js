...
http.createServer(async(req, res) => {
    ...
    if (pathname == '/autos' && req.method == 'DELETE'){
        const comicOriginales = await fs.readFile('autos.txt', 'utf8');
        const objetoComicsOriginal = JSON.parse(comicOriginales);
        const id = params.get('id');
        delete objetoComicsOriginal[id];

        //res.write(JSON.stringify(objetoComicsOriginal, null, 2));
        await fs.writeFile('autos.txt', JSON.stringify(objetoComicsOriginal, null, 2));
        res.write(`El autos con id ${id} ha sido eliminado exitosamente`);
        res.end();
    }
